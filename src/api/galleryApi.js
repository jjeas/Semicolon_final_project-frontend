...
const API_HOST_URL = "https://www.jaeseok.store/api/community/gallery";
...
export const fileRegister = async (file) => {
  ...
  const res = await axios.post(
    `https://www.jaeseok.store/api/upload/gallery`,
    formData
  );
  return res.data;
};
...
export const deleteFile = async (id) => {
  const res = await axios.delete(
    `https://www.jaeseok.store/api/upload/gallery/${id}`
  );
